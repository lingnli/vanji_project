                            ...
                            <div class="panel-body row">
                                ...
                                <div class="col-md-<?=$item[6] ?> col-sm-<?=$item[7] ?>">
                                    <div class="form-group">
                                        ...
                                        <? elseif($item[2] == "textarea"): ?>
                                        <textarea name="<?=$item[1] ?>" id="<?=$item[1] ?>" style="position: absolute; width: 1px; height: 1px; z-index: -100;"><?=$item[3] ?></textarea>
                                        <div class="summernote" id="<?=$item[1] ?>_sn" style="width: 100%; height: 350px;"><?=$item[3] ?></div>

                                        <? elseif($item[2] == "textarea_plain"): ?>
                                        <textarea class="form-control" id="<?=$item[1] ?>" name="<?=$item[1] ?>" style="height: 120px;"<?=(isset($can_edit)&&!$can_edit)?" disabled":"" ?>><?=$item[3] ?></textarea>
...
<script type="text/javascript" src="vendors/summernote/summernote-ext-dropdown.js"></script>
<script>
    <? if (isset($city)): ?>
    ...
    $(document).ready(function () {
        $(".summernote").summernote({
            height: <?=(isset($textarea_type) && $textarea_type == "mail")?'500':'200' ?>,
            toolbar: [
                ...
                ['height', ['height']],
                ['Insert', ['picture', 'link', 'table', 'hr', 'macroList']],
                ['misc', ['codeview']]
            ],
            template: {
            ...
            {
                // blockChar : '%%',
                items: ['會議名稱','買主名稱','廠商名稱'],
                // items1: [{value:'會議', content:'a'},
                //        {value:'attachement', content:'attachement'},
                //        {value:'address', content:'address'}],
            },
            hint: {
                // words: ['c','attachement','address'],
                //match: /^[%]([^]*)$/,
                match: /\B%(\w*)$/,
                blockChar_prefix : '{{',
                blockChar_aftfix : '}}',
                ...
                {
                return this.blockChar_prefix + item + this.blockChar_aftfix;
                }
            },
            callbacks: {
                onImageUpload: function(image) {
                    ...
                    $.ajax({
                        data: data,
                        type: "POST",
                        url: "<?=base_url() ?>dashboard/upload_pic",
                        cache: false,
                        contentType: false,
                        processData: false,
        ...
        $(".submit_btn").on('click', function(event) {
            <? if (isset($form_type) && $form_type == "ajax"): ?>
            // var formData = new FormData($('new_form').serialize());
            var formData = new FormData();
            
            $.each($('#new_form').serializeArray(), function(i, val) {
                formData.append(val.name, val.value);
            });

            $(document).find('.summernote').each(function(index, el) {
                var id = $(this).attr("id");
                var textarea_id = id.replace("_sn", "");

                formData.append(textarea_id, $('#'+id).summernote('code'));
            });
            ...
            $(document).find('.summernote').each(function(index, el) {
                ...
                $("#"+textarea_id).html($('#'+id).summernote('code'));
            });
            $("#new_form").submit();
            <? endif; ?>
        });
