...
class Member extends Base_Controller {
	...
	public function __construct(){
		...
		$this->param = [
                ...
                ["負責展代",				"event_group",		"select_multi",		"", 		TRUE, 	"", 	6, 		12, ["show_id", "title"]],
                ["SurveyCake 權限管理",	"sv_priv",			"select",			"", 		TRUE, 	"", 	6, 		12, ["id", "text"]],
            ];
	}
	...
	public function get_member_data(){
		...
		$member = $this->Priv_model->get_member_data($id);
		$this->output(TRUE, "success", array("member"=>$member));
	}
	...
	public function add(){
		...
		if ($_POST) {
			...
			$event_group = $this->input->post("event_group");

			$member_id = $this->Priv_model->member_add($data);
			if ($member_id !== FALSE) {
				$member_event_data = array();
				foreach ($event_group as $eg) {
					$member_event_data[] = array(
						"show_id" =>	$eg,
						"member_id"  =>	$member_id
					);	
				}
				$this->Priv_model->member_event_related($member_event_data);
			...
			foreach ($this->Event_model->get_event_group_list() as $eg) {
				$this->data['select']['event_group'][] = 	array(
					"show_id" =>	$eg['show_id'],
					"title"      =>	$eg['show_id']." ".$eg['name']
				);
			}
			$this->data['select']['sv_priv'] = array(
				array("id"=>"", "text"=>"尚未設定"),
				array("id"=>"admin", "text"=>"系統管理者（admin） - 公司帳號、管理者"),
				array("id"=>"boss", "text"=>"上級主管（boss） - 管理會員、編輯者"),
				array("id"=>"poweruser", "text"=>"展務人員（poweruser） - 專業會員、編輯者"),
				array("id"=>"user", "text"=>"展務助理（user） - 專業會員、編輯者")
			);
	...
	public function edit($id){
		...
		if ($_POST) {
			...
			unset($data['password_confirm']);
			unset($data['event_group']);
			$event_group = $this->input->post("event_group");
			$data['password'] = $this->encryption->encrypt(md5($data['password']));

			if ($this->Priv_model->member_edit($id, $data) !== FALSE) {
				$member_event_data = array();
				foreach ($event_group as $eg) {
					$member_event_data[] = array(
						"show_id" =>	$eg,
				...
				$this->Priv_model->clear_member_event_related($id);
				$this->Priv_model->member_event_related($member_event_data);
			...
			$data = $this->Priv_model->get_member_data($id);
			...
			$this->data['action'] = base_url()."mgr/member/edit/".$id;
			...
			$this->data['param'][9][3] = $this->Priv_model->get_member_event_related($id);
			$this->load->view("mgr/template_form_complex", $this->data);
		}
	}	
	...
	public function del(){
		...
		if ($this->Priv_model->member_edit($id, array("is_delete"=>1))) {
			$this->output(TRUE, "success");
		}else{
			$this->output(FALSE, "fail");
		}
