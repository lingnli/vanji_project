...
class Dashboard extends Base_Controller {
	...
	public function toggle_event($year, $event_uid = FALSE){
		// $show_list = $this->data['show_list'];
		
		if ($event_uid === FALSE) {
			// $selected = $show_list[$year][0];
		}else{
			// $index = array_search($event_uid, array_column($show_list[$year], 'event_uid'));
			// $selected = $show_list[$year][$index];	
		}
		
		// $this->session->set_userdata("selected_year", $year);
		// $this->session->set_userdata("selected_show_id", $selected['show_id']);
		// $this->session->set_userdata("selected_show_name", $selected['name']);
		// $this->session->set_userdata("selected_event_uid", $selected['event_uid']);
