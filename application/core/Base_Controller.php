...
class  Base_Controller  extends  CI_Controller  {
	...
	protected $matchweb_url = "https://matchweb.anbon.vip/tw/matchweb/";
	protected $o2oweb_url = "https://matchweb.anbon.vip/tw/o2oweb/";
	...
	protected $coupon_use = "coupon_use";
	...
	protected $brand_type_columns = array(
				array("id"=>"company", "text"=>"連鎖/企業"),
				// array("id"=>"person", "text"=>"單店/個人")
			);
	protected $meeting_type_columns = array(
				array("id"=>"onsite", "text"=>"現場會議"),
				array("id"=>"online", "text"=>"視訊會議"),
				array("id"=>"both", "text"=>"現場+視訊會議")
			);
	...
	public function __construct(){
		...
		if (strpos($_SERVER['REQUEST_URI'], '/mgr') !== FALSE) {
			...
			if ($this->session->selected_show_id != "" && $this->session->selected_event_uid != "") {
			...
			$this->data['work_time'] = $this->db->get_where("settings", array("id" => 1))->row()->content;
			$this->data['web_phone'] = $this->db->get_where("settings", array("id" => 3))->row()->content;
			$this->data['web_email'] = $this->db->get_where("settings", array("id" => 5))->row()->content;
		...
		$this->data['menu_product'] = (isset($menu_product)?$menu_product:array());
		$this->data['menu_total'] = (isset($menu_total) ? $menu_total : 0);
		$this->data['menu_count'] = (isset($menu_product) ? count($menu_product) : 0);
	...
	public function process_post_data($param){
		...
		foreach ($param as $item) {
			if ($item[2] == "" ||$item[2] == "show_img" || $item[2] == "header" || $item[2] == "select_multi" || $item[2] == "img_multi_without_crop" || $item[2] == "img_multi" || $item[2] == "plain") continue;
			...
			if ($item[4] == true && $this->input->post($item[1]) == "" && $item[1] != "images") {
				// print_r(111);
				// print_r($item);
				// print_r($this->input->post($item[1]));
				// exit;
				$this->js_output_and_back('請確認必填欄位皆填寫');
			}
