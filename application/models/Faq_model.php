...
class Faq_model extends Base_Model {
	...
	public function edit($id, $data, $is_multi = FALSE){
		if ($is_multi) {
			return $this->db->update_batch($this->faq, $data, $id);
		}else{
			return $this->db->where(array("id"=>$id))->update($this->faq, $data);
		}
	}

	public function add($data, $is_multi = FALSE){
		if ($is_multi) {
			return $this->db->insert_batch($this->faq, $data);
		}else{
	...
	public function get_data($id){
		return $this->db->get_where($this->faq, array("is_delete"=>0,'id'=>$id))->row_array();
	}
	...
	public function get_all_list($syntax = array()){
		...
		return $this->db->get_where($this->faq, $syntax)->result_array("event_uid");
	}

	public function get_list($syntax, $order_by, $page = 1, $page_count = 20){
		$total = $this->db->from($this->faq." N")
			// ->join("faq_classify C", "C.id = N.classify_id", "left")
