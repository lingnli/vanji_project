...
class EC_logistic extends CI_Model {
	/* anbon */
	// private $HashKey    = 'AVliFs00uGaz2Shx';
	// private $HashIV     = 'qJC2MZ5z310cDJ3W';
	// private $MerchantID = 3029327;
	/* anbon */
...
// print_r($trade_no);exit;
	        ...
	        if ($LogisticsType == "CVS") {
	        	...
	        	$AL->SendExtend = array(
					...
					// 'ReturnStoreID'   => $bill['storeID']
		        );
	        }else{
									//宅配
						//     	$this->db->select("M.zipcode, M.address");
						//     	$this->db->from("channels C");
						//     	$this->db->join("members M", "C.member_id = M.id", "left");
						//     	$this->db->where(array("C.id"=>$bill['channel_id']));
						//     	$m = $this->db->get()->row();

						//     	$distance = "00";
						//     	if (substr($m->zipcode, 0, 1) != substr($bill['zipcode'], 0, 1)) {
						//     		$distance = "01";
						//     	}

						//     	$sendExtend = array(
								// 'SenderZipCode'   => 	$m->zipcode,
								// 'SenderAddress'   => 	$m->address,
								// 'ReceiverZipCode' =>	$bill['zipcode'],
								// 'ReceiverAddress' =>	$bill['address'],
								// 'Temperature'     =>	'0001',
								// 'Distance'        =>	$distance,
								// 'Specification'   =>	'0001'
						//      );
						//      echo json_encode($sendExtend);
						//     	$AL->SendExtend = $sendExtend;
	        }
